...
export default function Home() {
  ...
  async function runAgents() {
    ...
    if (executionMode === "fast") {
      ...
      const fastInstruction = selected.length === 1 ? `Act as the ${agentCatalog.find((agent) => agent.id === selected[0])?.name}. Return one concise report with no more than five bullets.` : "Produce concise separate sections for these specialist agents with no more than five bullets per agent.";
      await requestReport(selected[0], `Pilot target readiness: ${readinessTarget}. ${fastInstruction} Prioritize the highest score recovery. ${JSON.stringify(compact)}`);
