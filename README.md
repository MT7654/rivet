...
Repository writes require an explicitly supplied fine-grained GitHub token. Rivet creates a separate branch and draft pull request, keeps the user inside an in-app PR workspace for review and checks, and merges only after explicit confirmation. It never pushes directly to the default branch or deploys automatically.

On the Pull Request screen, users paste their own token beginning with `github_pat_`. The in-app guide links to GitHub's token creator and explains how to select the repository and grant Contents and Pull requests read/write access. The token remains in browser memory through the PR workflow, is never persisted, and is cleared after merge.
...
- Target-driven orchestration: users choose Candidate (75+), Production Ready (90+), or Maximum. Rivet selects the supported pilot agents, shows each finding's score impact, and distinguishes original, projected, and validated readiness.
- Pilot boundary: the four-agent MVP reports its honest maximum achievable score and names the future specialist agents required when authentication, rate limiting, audit trails, or other controls prevent the repository from reaching its target.
...
- Pricing: GLM estimates use its verified routed rate; fallback-model cost is labelled unavailable when its exact routed rate cannot be verified.
